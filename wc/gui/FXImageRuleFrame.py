...
class FXImageRuleFrame (FXRuleFrame):
    ...
    def __init__ (self, parent, rule, index):
        ...
        FXLabel(matrix, _("Image width:\tThe width of the image to be blocked."), opts=LAYOUT_CENTER_Y|LAYOUT_LEFT)
        t = FXSpinner(matrix, 4, self, FXImageRuleFrame.ID_WIDTH, SPIN_NORMAL|FRAME_SUNKEN|FRAME_THICK)
        t.setRange(0,65535)
        t.setValue(self.rule.width)
        FXLabel(matrix, _("Image height:\tThe height of the image to be blocked."), opts=LAYOUT_CENTER_Y|LAYOUT_LEFT)
