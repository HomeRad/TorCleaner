...
def get_lang (headers, form):
    """look for lang attribute in form and header values"""
    if form and form.has_key('lang'):
        return i18n.get_lang(getval(form, "lang"))
    return i18n.get_headers_lang(headers)
...
def get_context (dirs, form, localcontext, lang):
    ...
    add_default_context(context, form, template, lang)
    # augment the context
...
def add_default_context (context, form, template, lang):
    # form values
    ...
    # standard macros
    path, dirs = get_template_path("macros/standard.html")
    f = file(path, 'r')
    macros = simpleTAL.compileHTMLTemplate(f)
    f.close()
    context.addGlobal("macros", macros)
    # used by navigation macro
    context.addGlobal("nav", {template: True})
    context.addGlobal("footer", i18n._("Generated by", lang=lang)+" "+App)
    # language
