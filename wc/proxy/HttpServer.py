...
def get_response_data (response, url):
    ...
    if len(parts)==2:
        bk.log.warn(wc.LOG_PROXY, "Empty response message from %r", url)
        parts += ['Bummer']
    elif len(parts)!=3:
        bk.log.error(wc.LOG_PROXY, "Invalid response %r from %r", response, url)
        parts = ['HTTP/1.0', 200, 'Ok']
    if not is_http_status(parts[1]):
        bk.log.error(wc.LOG_PROXY, "Invalid http statuscode %r from %r", parts[1], url)
        parts[1] = 200
    parts[1] = int(parts[1])
    return parts
...
def flush_decoders (decoders):
    ...
    data = ""
    while decoders:
        bk.log.debug(wc.LOG_PROXY, "flush decoder %s", decoders[0])
...
class HttpServer (wc.proxy.Server.Server):
    ...
    def process_connect (self):
        ...
        if self.client:
            ...
            wc.proxy.make_timer(0, lambda s=self: s.client and s.client.server_connected(s))
        else:
    ...
    def send_request (self):
        ...
        bk.log.debug(wc.LOG_PROXY, '%s write request\n%r', self, request)
        self.write(request)
        bk.log.debug(wc.LOG_PROXY, "%s write headers\n%s", self, self.clientheaders)
    ...
    def process_response (self):
        ...
        if self.response.lower().startswith('http'):
            ...
            wc.proxy.ServerPool.serverpool.set_http_version(self.addr, wc.proxy.get_http_version(protocol))
        elif not self.response:
            # It's a blank line, so assume HTTP/0.9
            bk.log.warn(wc.LOG_PROXY, "%s got HTTP/0.9 response", self)
            ...
            bk.log.warn(wc.LOG_PROXY, bk.i18n._('invalid or missing response from %r: %r'),
            ...
            if not wc.proxy.Headers.is_header(self.response):
                bk.log.warn(wc.LOG_PROXY, bk.i18n._("missing headers in response from %r"), self.url)
                self.recv_buffer = '\r\n' + self.recv_buffer
            # fix the response
            self.response = "%s 200 Ok"%self.protocol
            self.statuscode = 200
    ...
    def set_persistent (self, headers, http_ver):
        ...
        if http_ver >= (1,1):
            self.persistent = not wc.proxy.Headers.has_header_value(headers, 'Connection', 'Close')
        elif http_ver >= (1,0):
            self.persistent = wc.proxy.Headers.has_header_value(headers, 'Connection', 'Keep-Alive')
        else:
            self.persistent = False
    ...
    def process_content (self):
        ...
        bk.log.debug(wc.LOG_PROXY, "%s process %d bytes", self, len(data))
        if self.bytes_remaining is not None:
            # If we do know how many bytes we're dealing with,
            # we'll close the connection when we're done
            self.bytes_remaining -= len(data)
            bk.log.debug(wc.LOG_PROXY, "%s %d bytes remaining", self, self.bytes_remaining)
        is_closed = False
        for decoder in self.decoders:
            data = decoder.decode(data)
            bk.log.debug(wc.LOG_PROXY, "%s have run decoder %s", self, decoder)
        ...
        try:
            ...
            bk.log.debug(wc.LOG_PROXY, "%s FilterWait %s", self, msg)
        except wc.filter.FilterRating, msg:
            bk.log.debug(wc.LOG_PROXY, "%s FilterRating from content %s", self, msg)
            self._show_rating_deny(str(msg))
            return
        ...
        if underflow:
            bk.log.warn(wc.LOG_PROXY, bk.i18n._("server received %d bytes more than content-length"),
    ...
    def process_recycle (self):
        ...
        if self.statuscode==407 and wc.config['parentproxy']:
            bk.log.debug(wc.LOG_PROXY, "%s need parent proxy authentication", self)
            if self.authtries:
                ...
                return
            self.authtries += 1
            challenges = wc.proxy.auth.get_header_challenges(self.headers, 'Proxy-Authenticate')
            wc.proxy.Headers.remove_headers(self.headers, ['Proxy-Authentication'])
    ...
    def flush (self):
        ...
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.flush", self)
        if not self.statuscode:
            bk.log.warn(wc.LOG_PROXY, "%s flush without status", self)
    ...
    def set_readable (self, state):
        ...
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.set_readable", self)
        # the client might already have closed
        if self.client:
            self.state = state
            self.handle_close()
    ...
    def close_reuse (self):
        ...
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.close_reuse", self)
    ...
    def close_ready (self):
        """return True if connection has all data sent and is ready for
           closing"""
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.close_ready", self)
    ...
    def close_close (self):
        """close the connection socket and remove this connection from
           the connection pool"""
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.close_close", self)
    ...
    def handle_error (self, what):
        """tell the client that connection had an error, and close the
           connection"""
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.handle_error %r", self, what)
        if self.client:
    ...
    def reconnect (self):
        """reconnect to server"""
        bk.log.debug(wc.LOG_PROXY, "%s HttpServer.reconnect", self)
        # we still must have the client connection
        if not self.client:
            bk.log.error(wc.LOG_PROXY, "%s lost client on reconnect", self)
            return
...
def speedcheck_print_status ():
    ...
    if elapsed > 0 and SPEEDCHECK_BYTES > 0:
        bk.log.debug(wc.LOG_PROXY, 'speed: %4d b/s', (SPEEDCHECK_BYTES/elapsed))
        pass
    SPEEDCHECK_START = time.time()
    SPEEDCHECK_BYTES = 0
