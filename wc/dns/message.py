...
class Message(object):
    ...
    def __init__(self, id=None):
        if id is None:
            self.id = wc.dns.entropy.random_16()
        else:
            self.id = id
        self.flags = 0
        self.question = []
    ...
    def want_dnssec(self, wanted=True):
        ...
        if wanted:
            if self.edns < 0:
                self.use_edns()
            self.ednsflags |= wc.dns.flags.DO
        elif self.edns >= 0:
            self.ednsflags &= ~wc.dns.flags.DO
    ...
    def opcode(self):
        ...
        @rtype: int
        """
        return wc.dns.opcode.from_flags(self.flags)
    ...
    def set_opcode(self, opcode):
        ...
        """
        self.flags &= 0x87FF
        self.flags |= wc.dns.opcode.to_flags(opcode)
...
def make_response(query, recursion_available=False, our_payload=8192):
    ...
    @type query: wc.dns.message.Message object
    ...
    if recursion_available:
        response.flags |= wc.dns.flags.RA
