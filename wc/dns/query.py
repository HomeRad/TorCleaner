...
def _addresses_equal(af, a1, a2):
    ...
    n1 = wc.dns.inet.inet_pton(af, a1[0])
    n2 = wc.dns.inet.inet_pton(af, a2[0])
    ...
    try:
        ...
        while 1:
            ...
            if _addresses_equal(af, from_address, destination) or \
               (wc.dns.inet.is_multicast(where) and \
...
def xfr(where, zone, rdtype=wc.dns.rdatatype.AXFR,
        ...
        use_udp=False, keyalgorithm=wc.dns.tsig.default_algorithm):
    ...
    the one specified to wc.dns.zone.from_xfr().
    ...
    while not done:
        ...
        r = wc.dns.message.from_wire(wire, keyring=q.keyring, request_mac=q.mac,
                                  ...
                                  one_rr_per_rrset=(rdtype==wc.dns.rdatatype.IXFR))
        tsig_ctx = r.tsig_ctx
        first = False
        answer_index = 0
        delete_mode = False
        ...
        if soa_rrset is None:
            ...
            if rrset.rdtype != wc.dns.rdatatype.SOA:
                raise wc.dns.exception.FormError("first RRset is not an SOA")
            answer_index = 1
        ...
        for rrset in r.answer[answer_index:]:
            if done:
                raise wc.dns.exception.FormError("answers after final SOA")
            if rrset.rdtype == wc.dns.rdatatype.SOA and rrset.name == oname:
                if expecting_SOA:
                    if rrset[0].serial != serial:
                        raise wc.dns.exception.FormError("IXFR base serial mismatch")
                    expecting_SOA = False
                elif rdtype == wc.dns.rdatatype.IXFR:
