...
class NotAbsolute(wc.dns.exception.DNSException):
    """Raised if an absolute domain name is required but a relative name
    was provided."""
    pass

class NoRootSOA(wc.dns.exception.DNSException):
...
class Resolver(object):
            ...
            for l in f:
                ...
                if tokens[0] == 'nameserver':
                    ...
                    for suffix in tokens[1:]:
                        self.search.add(wc.dns.name.from_text(suffix))
        finally:
            if want_close:
                f.close()
    ...
    def read_local_ifaddrs(self):
        ...
        if os.name != 'posix':
            ...
            return []
        from wc.network import IfConfig
        ifc = IfConfig()
    ...
    def query(self, qname, rdtype=wc.dns.rdatatype.A,
              rdclass=wc.dns.rdataclass.IN, tcp=False, source=None):
        ...
        for qname in qnames_to_try:
            ...
            while response is None:
                ...
                for nameserver in nameservers[:]:
                    ...
                    if rcode != wc.dns.rcode.SERVFAIL:
                        nameservers.remove(nameserver)
                    response = None
    ...
    def use_tsig(self, keyring, keyname=None,
        ...
        is wc.dns.tsig.default_algorithm.
