...
class PeerError(wc.dns.exception.DNSException):
...
def get_algorithm(algorithm):
    ...
    try:
        import hashlib
        hashes[wc.dns.name.from_text('hmac-sha224')] = hashlib.sha224
        hashes[wc.dns.name.from_text('hmac-sha256')] = hashlib.sha256
        hashes[wc.dns.name.from_text('hmac-sha384')] = hashlib.sha384
        hashes[wc.dns.name.from_text('hmac-sha512')] = hashlib.sha512
        hashes[wc.dns.name.from_text('hmac-sha1')] = hashlib.sha1
        hashes[wc.dns.name.from_text('HMAC-MD5.SIG-ALG.REG.INT')] = hashlib.md5
        ...
        import md5, sha
        hashes[wc.dns.name.from_text('HMAC-MD5.SIG-ALG.REG.INT')] =  md5.md5
        hashes[wc.dns.name.from_text('hmac-sha1')] = sha.sha

    if isinstance(algorithm, (str, unicode)):
        algorithm = wc.dns.name.from_text(algorithm)
