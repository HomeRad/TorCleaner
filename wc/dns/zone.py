...
class _MasterReader(object):
    ...
    def _rr_line(self):
        ...
        if not token.is_whitespace():
            self.last_name = wc.dns.name.from_text(token.value, self.current_origin)
        else:
            token = self.tok.get()
            if token.is_eol_or_eof():
        ...
        # TTL
        try:
            ttl = wc.dns.ttl.from_text(token.value)
            token = self.tok.get()
            if not token.is_identifier():
                raise wc.dns.exception.DNSSyntaxError
        ...
        try:
            ...
            raise wc.dns.exception.DNSSyntaxError("unknown rdatatype '%s'" % token.value)
        ...
        try:
            ...
            (ty, va) = sys.exc_info()[:2]
            raise wc.dns.exception.DNSSyntaxError("caught exception %s: %s" % (str(ty), str(va)))
            ...
            while 1:
                ...
                if token.is_eof():
                    ...
                    if u == '$TTL':
                        ...
                        self.ttl = wc.dns.ttl.from_text(token.value)
                        self.tok.get_eol()
                    elif u == '$ORIGIN':
                        ...
                        if not token.is_quoted_string():
                            raise wc.dns.exception.DNSSyntaxError("bad filename in $INCLUDE")
                        filename = token.value
                        token = self.tok.get()
                        if token.is_identifier():
                            new_origin = wc.dns.name.from_text(token.value, \
                                                        self.current_origin)
                            self.tok.get_eol()
                        elif not token.is_eol_or_eof():
                            raise wc.dns.exception.DNSSyntaxError("bad origin in $INCLUDE")
                        else:
                        ...
                        raise wc.dns.exception.DNSSyntaxError("Unknown master file directive '" + u + "'")
                    continue
                self.tok.unget(token)
                self._rr_line()
...
def from_text(text, origin, rdclass = wc.dns.rdataclass.IN,
