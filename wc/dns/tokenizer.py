...
class UngetBufferFull(wc.dns.exception.DNSException):
...
class Tokenizer(object):
    ...
    def get_uint8(self):
        ...
        if value < 0 or value > 255:
            raise wc.dns.exception.DNSSyntaxError('%d is not an unsigned 8-bit integer' % value)
        return value
    ...
    def get_uint16(self):
        ...
        if value < 0 or value > 65535:
            raise wc.dns.exception.DNSSyntaxError('%d is not an unsigned 16-bit integer' % value)
        return value
    ...
    def get_uint32(self):
        ...
        if value < 0 or value > 4294967296L:
            raise wc.dns.exception.DNSSyntaxError('%d is not an unsigned 32-bit integer' % value)
        return value
    ...
    def get_name(self, origin=None):
        ...
        if not token.is_identifier():
            raise wc.dns.exception.DNSSyntaxError('expecting an identifier')
        return wc.dns.name.from_text(token.value, origin)
    ...
    def get_eol(self):
        ...
        if not token.is_eol_or_eof():
            raise wc.dns.exception.DNSSyntaxError('expected EOL or EOF, got %d "%s"' % (token.ttype, token.value))
        return token.value
    ...
    def get_ttl(self):
        ...
        if not token.is_identifier():
            raise wc.dns.exception.DNSSyntaxError('expecting an identifier')
        return wc.dns.ttl.from_text(token.value)
