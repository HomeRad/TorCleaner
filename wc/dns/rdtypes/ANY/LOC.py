...
def _exponent_of(what, desc):
    ...
    if exp is None or exp < 0:
        raise wc.dns.exception.DNSSyntaxError("%s value out of bounds" % desc)
    return exp
...
def _decode_size(what, desc):
    exponent = what & 0x0F
    if exponent > 9:
        raise wc.dns.exception.DNSSyntaxError("bad %s exponent" % desc)
    base = (what & 0xF0) >> 4
    if base > 9:
        raise wc.dns.exception.DNSSyntaxError("bad %s base" % desc)
    return long(base) * pow(10, exponent)

class LOC(wc.dns.rdata.Rdata):
    """LOC record
    ...
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        ...
        if t.isdigit():
            ...
            if '.' in t:
                ...
                if not seconds.isdigit():
                    raise wc.dns.exception.DNSSyntaxError('bad latitude seconds value')
                latitude[2] = int(seconds)
                if latitude[2] >= 60:
                    raise wc.dns.exception.DNSSyntaxError('latitude seconds >= 60')
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise wc.dns.exception.DNSSyntaxError('bad latitude milliseconds value')
                if l == 1:
                    m = 100
                elif l == 2:
                    m = 10
        ...
        if t == 'S':
            ...
            raise wc.dns.exception.DNSSyntaxError('bad latitude hemisphere value')
        ...
        if t.isdigit():
            ...
            if '.' in t:
                (seconds, milliseconds) = t.split('.')
                if not seconds.isdigit():
                    raise wc.dns.exception.DNSSyntaxError('bad longitude seconds value')
                longitude[2] = int(seconds)
                if longitude[2] >= 60:
                    raise wc.dns.exception.DNSSyntaxError('longitude seconds >= 60')
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise wc.dns.exception.DNSSyntaxError('bad longitude milliseconds value')
                if l == 1:
                    m = 100
                elif l == 2:
                    m = 10
        ...
        if t == 'W':
            ...
            raise wc.dns.exception.DNSSyntaxError('bad longitude hemisphere value')
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        if latitude < -90.0 or latitude > 90.0:
            raise wc.dns.exception.FormError("bad latitude")
        ...
        if longitude < -180.0 or longitude > 180.0:
            raise wc.dns.exception.FormError("bad longitude")
