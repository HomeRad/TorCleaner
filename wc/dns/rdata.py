...
class Rdata(object):
    ...
    def validate(self):
        ...
        changes.
        """
        wc.dns.rdata.from_text(self.rdclass, self.rdtype, self.to_text())
...
class GenericRdata(Rdata):
    ...
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        ...
        if not token.is_identifier() or token.value != '\#':
            raise wc.dns.exception.DNSSyntaxError(r'generic rdata does not start with \#')
        length = tok.get_int()
        chunks = []
        while 1:
            token = tok.get()
        ...
        if len(data) != length:
            raise wc.dns.exception.DNSSyntaxError('generic rdata hex data has wrong length')
