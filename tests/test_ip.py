...
class TestIp (unittest.TestCase):
    ...
    def testNames (self):
        ...
        for host in iputil.resolve_host("www.kampfesser.net"):
            self.assert_(iputil.host_in_set(host, hosts, nets))
        self.assert_(not iputil.host_in_set("q2345qwer9 u2 42ß3 i34 uq3tu ", hosts, nets))
        self.assert_(not iputil.host_in_set("q2345qwer9", hosts, nets))
        self.assert_(not iputil.host_in_set("2.3.4", hosts, nets))
        self.assert_(not iputil.host_in_set(".3.4", hosts, nets))
        self.assert_(not iputil.host_in_set("3.4", hosts, nets))
        self.assert_(not iputil.host_in_set(".4", hosts, nets))
        self.assert_(not iputil.host_in_set("4", hosts, nets))
        self.assert_(not iputil.host_in_set("", hosts, nets))

    def testIPv4_1 (self):
        hosts, nets = iputil.hosts2map(["1.2.3.4"])
        self.assert_(iputil.host_in_set("1.2.3.4", hosts, nets))

    def testBitmask0 (self):
        """test the catch-all ip bitmask"""
        hosts, nets = iputil.hosts2map(["192.168.1.1/0"])
        self.assert_(iputil.host_in_set("1.1.1.1", hosts, nets))
        self.assert_(iputil.host_in_set("0.0.0.0", hosts, nets))
        self.assert_(iputil.host_in_set("77.88.33.1", hosts, nets))
        self.assert_(iputil.host_in_set("255.255.255.255", hosts, nets))

    def testBitmask8 (self):
        hosts, nets = iputil.hosts2map(["192.168.1.1/8"])
        for i in xrange(255):
            self.assert_(iputil.host_in_set("192.168.1.%d"%i, hosts, nets))

    def testBitmask16 (self):
        hosts, nets = iputil.hosts2map(["192.168.1.1/16"])
        for i in xrange(255):
            for j in xrange(255):
                self.assert_(iputil.host_in_set("192.168.%d.%d"%(i,j), hosts, nets))

    def testBitmask32 (self):
        hosts, nets = iputil.hosts2map(["192.168.1.1/32"])
        self.assert_(not iputil.host_in_set("1.1.1.1", hosts, nets))
        self.assert_(not iputil.host_in_set("0.0.0.0", hosts, nets))
        self.assert_(not iputil.host_in_set("77.88.33.1", hosts, nets))
        self.assert_(iputil.host_in_set("255.255.255.255", hosts, nets))
